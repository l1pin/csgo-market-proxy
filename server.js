...
// Модификация URL в контенте (исправленная версия без window)
function modifyUrls(content, baseUrl, contentType = '') {
    ...
    if (contentType.includes('html')) {
        ...
        if (!modified.includes('upgrade-insecure-requests')) {
            modified = modified.replace(/<head[^>]*>/i, `// Модификация URL в контенте
function modifyUrls(content, baseUrl, contentType = '') {
    if (!content) return content;
...
// Модификация функции, которая может использовать window
...
// ИСПРАВЛЕНО: Улучшенная обработка GraphQL запросов без ссылок на window
...
// ИСПРАВЛЕНО: Улучшен основной обработчик HTTP запросов без использования window
